...
def tidal_const_init():
    ...
    global m2ang, s2ang, n2ang, k2ang, k1ang, o1ang, tol
    global angVels, phases
    ...
    # Column names
    fp = open("tidalconst/fvcom_tidal_const.csv", "r")
    headerline = fp.readline()
    
    csvline = fp.readline()
    
    northy=36651.1955983
    southy=0.0
    westx=0.0
    eastx=21453.7278466

    while (csvline != ""):
        (x, y, m2amp, m2phase, s2amp, s2phase, n2amp, n2phase, \
        ...
        if ("m2" not in tidalConstituents):
            m2amp=0.0
            
        if ("s2" not in tidalConstituents):
            s2amp=0.0
            
        if ("k2" not in tidalConstituents):
            k2amp=0.0
            
        if ("n2" not in tidalConstituents):
            n2amp=0.0
            
        if ("k1" not in tidalConstituents):
            k1amp=0.0
            
        if ("o1" not in tidalConstituents):
            o1amp=0.0
            
        if ("p1" not in tidalConstituents):
            p1amp=0.0

        if ("q1" not in tidalConstituents):
            q1amp=0.0
        ...
        if(len(foundIn)>0):
            pt=BoundaryPoint(x, y, m2amp, m2phase, s2amp, s2phase, \
...
def init(boundary, constituents=("m2", "s2", "n2", "k2", "k1", "o1" ), \
         ramptimehours=48.0, scale=1.0):
    global boundaryName, ramptime, boundaryPts
    global scaleperm, tidalConstituents
